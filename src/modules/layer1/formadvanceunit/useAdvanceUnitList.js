...
const useAdvanceUnitList = () => {
    ...
    const handleList = useCallback((key) => {
        const url = key 
            ? `${config[process.env.REACT_APP_ENV].API_URL_advance_unit_list}/${key}` // Búsqueda por ID
            : config[process.env.REACT_APP_ENV].API_URL_advance_unit_list; // Lista completa
...
export default useAdvanceUnitList;