...
const useWeekCurrenYearLogList = () => {
    ...
    const handleList = useCallback((key) => {
        const url = key 
            ? `${config[process.env.REACT_APP_ENV].API_URL_catch_week_current_year_list}/${key}` // Búsqueda por ID
            : config[process.env.REACT_APP_ENV].API_URL_catch_week_current_year_list // Lista completa
...
export default useWeekCurrenYearLogList;