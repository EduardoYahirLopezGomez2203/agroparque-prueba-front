...
const useTypeActivityList = () => {
    ...
    const handleList = useCallback((key) => {
        const url = key 
            ? `${config[process.env.REACT_APP_ENV].API_URL_type_activity_list}/${key}` // Búsqueda por ID
            : config[process.env.REACT_APP_ENV].API_URL_type_activity_list; // Lista completa
...
export default useTypeActivityList;