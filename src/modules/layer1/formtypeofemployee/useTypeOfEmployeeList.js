...
const useTypeOfEmployeeList = () => {
    ...
    const handleList = useCallback((key) => {
        const url = key 
            ? `${config[process.env.REACT_APP_ENV].API_URL_type_of_employee_list}/${key}` // Búsqueda por ID
            : config[process.env.REACT_APP_ENV].API_URL_type_of_employee_list; // Lista completa
...
export default useTypeOfEmployeeList;