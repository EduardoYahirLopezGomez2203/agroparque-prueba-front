...
const useEmployeeLabourList = () => {
    ...
    const handleList = useCallback((key) => {
        const url = key 
            ? `${config[process.env.REACT_APP_ENV].API_URL_labour_list}/${key}`
            : config[process.env.REACT_APP_ENV].API_URL_labour_list;
...
export default useEmployeeLabourList;