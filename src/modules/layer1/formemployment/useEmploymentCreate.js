...
const useEmploymentCreate = () => {
    ...
    const handleCreate = async (dataValue,isUpdate, handleFormUpdate) => {
        if(!isUpdate){
        const url = config[process.env.REACT_APP_ENV].API_URL_employment_create;
        console.log("Datos a enviar a la API:", dataValue);  
        await obtenerDatos('POST', url, dataValue);  
        console.log("URL:", url);
        console.log("Puesto creado existosamente");
        } else {
            handleFormUpdate();
        }
    };
...
export default useEmploymentCreate;
