...
const useProfileCreate = () => {
    ...
    const handleCreate = async (dataValue,isUpdate, handleFormUpdate) => {
        if(!isUpdate){
        const url = config[process.env.REACT_APP_ENV].API_URL_profile_create;
        console.log("Datos a enviar a la API:", dataValue);  
        await obtenerDatos('POST', url, dataValue);  
        console.log("URL:", url);
        console.log("Perfil creado existosamente");
        } else {
            handleFormUpdate();
        }
    };
...
export default useProfileCreate;
