...
const DinamicTableComponent = ({
    information = { header: [], body: [] }, isLoading = false, error = null, menuOptions = []
}) => {
                    ...
                    <TableBody>
                        {informationPerPage.map((row) => (
                            <TableRow key={row.id}>
                                ...
                                <TableCell sx={{ height: menuOptions.length === 0 ? '50px' : '0'}} align="right">
                                    {menuOptions.length > 0 && 
                                    <MenuComponent menuOptions={menuOptions} row={row} />
                                    }
                                </TableCell>
                            </TableRow>
                        ))}
...
const MenuComponent = ({ menuOptions = [], row }) => {
                ...
                <MenuList disablePadding >
                    {menuOptions.map((option, index) => (
                        <MenuItem
                            ...
                            onClick={() => {
                                setAnchorEl(null);
                                // Si la función espera un argumento, pásale la fila
                                if (option.onClick.length > 0) {
                                    option.onClick(row);
                                } else {
                                    option.onClick();
                                }
...
export default DinamicTableComponent;
